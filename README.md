...
1. Самосвал: Truck(bool init_location, int number, int weight, int speed, int bridge_length);
2. Регулировщик: TrafficController(bool location, int max_weight, int max_number_of_trucks).
...
3. Во второй строчке количество грузовиков (пусть n);
4. Далее в идёт n строчек вида ``